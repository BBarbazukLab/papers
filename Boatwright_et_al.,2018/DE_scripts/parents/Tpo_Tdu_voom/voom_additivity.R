...
setwd('/ufrc/barbazuk/lboat/Old_World_New_BED/trago_output/ase_counts/parents/Tpo_Tdu_voom')
targets <- readTargets()
print(targets)
...
print(dim(d))
#print(head(d))
colnames(d) <- c("Tdu1","Tdu2","Tdu3","Tpo1","Tpo2","Tpo3","Tm1","Tm2","Tm3")
...
trt = factor(c(1,1,1,2,2,2,3,3,3), labels=c("Tdu","Tpo","Tm"))
...
write.table(top, "DE_Tdu_Tpo.txt", sep="\t", quote=F, row.names = TRUE)

contrast.matrix=makeContrasts(contrasts=c("(trtTdu+trtTpo)/2-trtTm"), levels=design)
fit2=contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)
...
write.table(top, "DE_additive_Tdu_Tpo-Tm.txt", sep="\t", quote=F, row.names = TRUE)
