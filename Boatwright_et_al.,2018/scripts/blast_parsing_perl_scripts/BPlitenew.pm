...
sub nextHSP {
	...
	my ($frame) = $scoreline =~/Frame\s+=\s+([+-]\d)/;  #added to pull fram nformation
	if (not defined $p) {
	...
	my $hsp = BPlite::HSP::new( $score, $bits, $match, $positive, $length, $p,
		$qb, $qe, $sb, $se, $ql, $sl, $as, $qgaps, $sgaps, $frame); #added frame var
	return $hsp;
}
...
sub new {
	...
	($hsp->{SCORE}, $hsp->{BITS},
		...
		$hsp->{QL}, $hsp->{SL}, $hsp->{AS}, $hsp->{QG}, $hsp->{SG}, $hsp->{FRAME}) = @_;
