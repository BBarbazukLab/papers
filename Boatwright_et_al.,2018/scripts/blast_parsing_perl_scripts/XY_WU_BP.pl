...
BEGIN   {
        push (@INC, "/scratch/lfs/mcintyre/trago/trago_output/trago_blast_dbs/BLASTN_output/Chen_data/WU_BP_parameter_checks/X_Y_test/");
}
	...
	# print "$qname $qlength\n";
	 ...
	 SUBJECT:while(my $subject = $blast->nextSbjct) {
		 ...
		 while(my $hsp = $subject->nextHSP) {
			...
			my $allowed_overhang = 50;
			my $cons = ($pos/$alig_len)*100;
		 	next if ($qname eq $sname);
			$alig_p =~ s/\,//g; $qlength =~ s/\,//g;
			my $alig_test = $alig_p/$qlength;
			next unless ($id >= 98) && ($pval <= 1e-100) && (abs($se-$sb)/$slength > 0.9);# && ($alig_test > 0.9); 
			#my $xyFLAG = XY($qlength, $slength, $qb, $qe, $sb, $se, $strand, $allowed_overhang); 
			#next unless ($xyFLAG == 0);#($qlength >= 500) && ($slength>=500) && ($xyFLAG == 0); #&& ($qlength <= 5000) && ($slength<=5000);
